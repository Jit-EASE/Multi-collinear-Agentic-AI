# Update the unified suite so it (1) clearly uses Gemini when heterogeneous mode is selected,
# (2) shows per-agent API labels, (3) adds a regression chart with coefficient CIs,
# and (4) includes a VIF bar chart (already present) plus a quick Gemini connectivity check.
#
# agentic_ai_multicollinearity_suite_v2.py
# ------------------------------------------------------------------
# Agentic AI Multicollinearity Suite — v2
# Adds explicit Gemini usage, API badges, OLS regression chart (coefficients + 95% CI),
# and keeps VIF/Correlation/PCA diagnostics.
...
# ===== UI CONFIG =====
st.set_page_config(page_title="Agentic AI Multicollinearity Suite v2", page_icon="🧮", layout="wide")

HEADER = """
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.8rem;">
  <div>
    <h1 style="margin:0;">Agentic AI Multicollinearity Suite — v2</h1>
...
with st.expander("Quick Start", expanded=False):
    ...
    **Run**
    ```bash
    streamlit run agentic_ai_multicollinearity_suite_v2.py
    ```
    """)
...
def assign_models(n_agents: int, heterogeneous: bool, force_provider: str=None) -> List[str]:
    """
    If force_provider in {"openai","gemini"}, use only that provider for all agents (debugging).
    Otherwise: in heterogeneous mode, alternate across available providers.
    """
    providers = []
...
def run_general_lab(cfg: dict) -> dict:
    ...
    use_roles = bool(cfg["use_roles"]); debate = bool(cfg.get("debate", False))
    ...
    rows, meta, provider_log = [], [], []
    for p in prompts:
        a_scores, a_meta, a_prov = [], [], []
        for a in range(n_agents):
            ...
            a_prov.append(prov)
        rows.append(a_scores); meta.append(a_meta); provider_log.append(a_prov)
    ...
    # ---- Regression target (for visualization only) ----
    # Use a trimmed mean of agent scores with small noise so coefficients exist but
    # collinearity leads to wide CIs in homogeneous mode.
    ...
    conf = model.conf_int().values[1:]  # exclude constant
    se = (params - conf[:,0]) / 1.96  # approximate
    ...
    return {"X":X, "corr":corr, "vif":vif, "cond":cidx, "pc1":pc1,
            "providers":providers, "meta":meta, "provider_log":provider_log,
            "reg_df": reg_df, "reg_summary": model.summary().as_text()}
...
def run_agri_lab(cfg: dict) -> dict:
    ...
    for i in range(n_fields):
        ...
        for a in range(n_agents):
            ...
            a_prov.append(prov)
        rows.append(a_scores); raw.append({"field":card, "agents":a_json}); provider_log.append(a_prov)

    X = pd.DataFrame(rows, columns=[f"Agent{i+1}" for i in range(n_agents)])
    corr = X.corr(numeric_only=True)
    vif = safe_vif(X); cidx = condition_index(X); pc1 = pca_first_share(X)

    # ---- Regression target for visualization ----
    # Build a plausible yield proxy from the field card for realism.
    # For simplicity, reconstruct from 'raw' structure (ndvi dominant, penalize water stress proxy).
    yield_proxy = []
    for rec in raw:
        f = rec["field"]
        # crude water stress proxy: high ET0 + low moisture - recent rain
        stress = max(0.0, (f["ET0_mm_day"]*10 - f["soil_moisture_pct"]) - 0.2*f["rain_last_7d_mm"])
        y = 20 + (f["ndvi"]*100)*0.8 - 0.3*stress + np.random.normal(0, 2.0)
        yield_proxy.append(y)
    y = np.array(yield_proxy)

    X_ols = sm.add_constant(X.values)
    model = sm.OLS(y, X_ols, hasconst=True).fit()
    params = model.params[1:]
    conf = model.conf_int().values[1:]
    se = (params - conf[:,0]) / 1.96
    ...
    return {"X":X, "corr":corr, "vif":vif, "cond":cidx, "pc1":pc1,
            "providers":providers, "raw":raw, "provider_log":provider_log,
            "reg_df": reg_df, "reg_summary": model.summary().as_text()}
...
with tab1:
    ...
    if run_g:
        ...
        providers, provider_log = res["providers"], res["provider_log"]
        reg_df, reg_summary = res["reg_df"], res["reg_summary"]
...
with tab3:
    st.markdown("""
    ### About & Methods
    **Goal.** Show that homogeneous, single-LLM multi-agent systems emit highly correlated signals (inflating multicollinearity),
    whereas heterogeneous, multi-LLM committees produce more diverse signals (lower collinearity).
    
    **New in v2.**
    - Explicit provider alternation and badges (so you can verify Gemini is active).
    - Debug knob to force Gemini-only or OpenAI-only runs.
    - **Regression charts** with 95% CI to visualize coefficient instability under collinearity.
    
    **Metrics.**
    - Correlation matrix, VIF per agent variable (VIF > 10 severe), Condition Index, PCA PC1 share.
    
    **Notes.**
    - Regression targets here are for *visualization only* (a synthetic target in General; a yield proxy from field features in Agriculture).
      Multicollinearity primarily affects **standard errors** and coefficient stability; you will see wider CIs in homogeneous runs.
    """)

st.markdown("<hr/>", unsafe_allow_html=True)
st.markdown("<div style='text-align:center;opacity:0.7;'>© Designed & Developed by Jit — Agentic AI Multicollinearity Suite v2</div>", unsafe_allow_html=True)
